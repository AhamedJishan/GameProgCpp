...
namespace jLab
{
	...
	bool Shader::Compile(const std::string& shaderSource, GLenum shaderType, GLuint& outShader)
	{
		...
		if (file.is_open())
		{
			...
			{
				SDL_Log("ERROR: Failed to compile shader %s", shaderSource.c_str());
				return false;
			}
		}
		else
		{
			SDL_Log("ERROR: Failed to Open file %s", shaderSource.c_str());
			return false;
		}
		return true;
	}
